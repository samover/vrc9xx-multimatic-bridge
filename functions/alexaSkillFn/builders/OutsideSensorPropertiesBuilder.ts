...
export class OutsideSensorPropertiesBuilder implements PropertiesBuilder {
    public build(system: SystemStatusModel): ContextProperty[] {
        const timestamp = (new Date()).toISOString();

        return [
            OutsideSensorPropertiesBuilder.buildTemperatureSensorBlock(system, timestamp),
            OutsideSensorPropertiesBuilder.buildConnectivityBlock(timestamp),
        ];
    }
